...
class GalleryDataset(Dataset):
    """Gallery dataset returning raw images + paths for evaluation."""
    ...
    def __init__(self, data_path: Union[str, os.PathLike], split: str,
        ...
        if dataset in ['cirr', 'cirr_r']:
            with open(os.path.join(data_path, 'image_splits', f'split.rc2.{split}.json'), "r") as f:
                mapper = json.load(f)
            self.img_ids = sorted(list(mapper.keys()))
            self.image_paths = [os.path.join(data_path, 'img_raw', mapper[img_id].strip('./')) for img_id in self.img_ids]
            
        elif dataset == 'hotels':
            img_dir = 'train' if split == 'train' else 'val_test'
            dataset_path = os.path.join(data_path, 'captions', f'hotel_{split}_data.json')
            with open(dataset_path, "r") as f:
                imgs_info = json.load(f)
                
            unique_paths = set()
            for img_info in imgs_info:
                q_path = os.path.join(data_path, 'images', img_dir, *img_info["query_image"].split('/')[-3:])
                r_path = os.path.join(data_path, 'images', img_dir, *img_info["result_image"].split('/')[-3:])
                unique_paths.add(q_path)
                unique_paths.add(r_path)
                
            self.image_paths = sorted(list(unique_paths))
            
        print(f"GalleryDataset initialized with {len(self.image_paths)} images")
    ...
    def __getitem__(self, index) -> dict:
        try:
            ...
            print(f"Error loading gallery image at index {index}: {e}")
